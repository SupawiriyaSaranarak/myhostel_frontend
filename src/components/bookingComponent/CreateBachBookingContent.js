...
function CreateBachBookingContent() {
  ...
  const handleGetAllAccomodation = async (e) => {
    e.preventDefault();
    console.log("123456");
    try {
      const res = await axios.get(
        `/admin/accomodations/validinoutdate?checkinDate=${input.checkinDate}&checkoutDate=${input.checkoutDate}`
      );
      console.log(res.data.getBookingItems);
      setResult(res.data.getBookingItems);
    } catch (err) {
      console.dir(err);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const parseAccomodationId = accomodationId.map((item) =>
        JSON.parse(item)
      );
      console.log(parseAccomodationId);
      const req = {
        ...input,
        accomodationId: parseAccomodationId,
        paymentImg: productImg,
      };
      console.log(req);
      const response = await axios.post("/admin/batchbooking", req);
            ...
            <div className="form-group">
              ...
              {!result.length && (
                <button
                  ...
                  onClick={handleGetAllAccomodation}
                >
                  Get Accomodation
                </button>
              )}
                  ...
                  <>
                    ...
                    {item.getBookingItem.map((acc) => (
                      <>
                        ...
                        />
                        &nbsp;
                        <label htmlFor={acc.id}>{acc.id}</label>
                        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                      </>
                    ))}
                  </>
...
export default CreateBachBookingContent;
